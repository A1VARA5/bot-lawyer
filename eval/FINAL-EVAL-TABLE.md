...
## Re run after the provenance fix (2026-09-21)

q21 (the FAIL), q24, k02 and u02 were re run after the prompt rule that blockquotes may only contain a rule's quote field or a page's own words. Result: 12 blockquotes across the 4 answers, 12 of 12 found verbatim in the raw page snapshots by normalised string match; 0 em or en dashes; u02 still admits the gap. Files: results-refix.json, results-refix.md. The table above is unchanged on purpose; this section is the measurement of the fix.
